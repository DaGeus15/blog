...
if(isset($_POST['add_comment'])){
   ...
   $verify_comment = $conn->prepare("SELECT * FROM `comments` WHERE post_id = ? AND post_owner_id = ? AND commenter_id = ? AND user_name = ? AND comment = ?");
   $verify_comment->execute([$get_id, $admin_id, $user_id, $user_name, $comment]);

   if($verify_comment->rowCount() > 0){
      $message[] = 'Comentario ya agregado!';
   }else{
      $insert_comment = $conn->prepare("INSERT INTO `comments`(post_id, post_owner_id, commenter_id, user_name, comment) VALUES(?,?,?,?,?)");
...
<head>
   ...
   <title>ShadowWilds-Ver Blog</title>
...
?>
   ...
   <form action="" method="POST">
      ...
      <textarea name="comment_edit_box" required cols="30" rows="10" placeholder="Ingresa tu comentario..."><?= $fetch_edit_comment['comment']; ?></textarea>
      <button type="submit" class="inline-btn" name="edit_comment">Editar</button>
      <div class="inline-option-btn" onclick="window.location.href = 'view_post.php?post_id=<?= $get_id; ?>';">Cancelar Edicion</div>
   </form>
   </section>
<?php
   }
...
<section class="posts-container" style="padding-bottom: 0;">
   ...
   <div class="box-container">
      ...
      <?php
         ...
         echo '<p class="empty">No se encontraron blogs!</p>';
      }
      ?>
   </div>
...
<section class="comments-container">
   ...
   <p class="comment-title">Agregar Comentario</p>
   <?php
      if($user_id != ''){  
   ...
   <div class="add-comment">
      <p>Inicia Sesion para comentar o reaccionar</p>
      <a href="public/login_public.php" class="inline-btn">Iniciar Sesion</a>
   </div>
   <?php
      }
   ?>
   ...
   <div class="user-comments-container">
      ...
      ?>
      <div class="show-comments" style="<?php if($fetch_comments['commenter_id'] == $user_id){echo 'order:-1;'; } ?>">
         ...
         <div class="comment-box" style="<?php if($fetch_comments['commenter_id'] == $user_id){echo 'color:var(--white); background:var(--black);'; } ?>"><?= $fetch_comments['comment']; ?></div>
         <?php
            if($fetch_comments['commenter_id'] == $user_id){  
         ?>
         <form action="" method="POST">
            <input type="hidden" name="comment_id" value="<?= $fetch_comments['id']; ?>">
            <button type="submit" class="inline-option-btn" name="open_edit_box">Editar Comentario</button>
            <button type="submit" class="inline-delete-btn" name="delete_comment" onclick="return confirm('¿Elimnar comentario?');">Eliminar comentario</button>
         </form>
         <?php
         }
         ?>
